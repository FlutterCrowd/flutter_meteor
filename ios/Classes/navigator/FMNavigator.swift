...
//        print("Call push untilRouteName:\(routeName)")
    ...
    public static func popUntil(untilRouteName: String, options: FMMeteorOptions?) {
        ...
        FlutterMeteorRouter.searchRoute(routeName: untilRouteName) { viewController in
            _popUntil(untilRouteName: untilRouteName, options: options, untilPage: viewController)
        }
    }
    
    private static func _popUntil(untilRouteName: String, options: FMMeteorOptions?, untilPage: UIViewController?) {
    ...
    public static func _realPushToAndRemoveUntil(routeName: String, untilRouteName: String?, untilPage: UIViewController?, options: FMMeteorOptions?) {
        ...
        if (toPage != nil) {
            if(untilPage is FlutterViewController) {
                FMNativeNavigator.push(toPage: toPage!)
                let channel = FlutterMeteor.methodChannel(flutterVc: untilPage as! FlutterViewController)
                if(channel != nil) {
                    var arguments: Dictionary<String, Any?> = options?.arguments ?? [:]
                    if (arguments["routeName"] == nil) {
                        arguments["routeName"] = untilRouteName
    ...
    public static func pushNamedAndRemoveUntilRoot(routeName: String, options: FMMeteorOptions?) {
